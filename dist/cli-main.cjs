  ...
  "node_modules/.pnpm/picomatch@4.0.4/node_modules/picomatch/lib/picomatch.js"(exports2, module2) {
    ...
    var picomatch2 = (glob, options, returnState = false) => {
      ...
      const matcher = (input, returnObject = false) => {
        const { isMatch, match, output: output2 } = picomatch2.test(input, regex, options, { glob, posix: posix2 });
        const result = { glob, state, regex, posix: posix2, input, output: output2, match, isMatch };
...
// src/audience.ts
var AGENT_CONTEXT_FILES = /* @__PURE__ */ new Set([
  "CLAUDE.md",
  "AGENTS.md",
  "CONTEXT.md"
]);
...
function parseDocmeta(content, relPath) {
  const match = content.match(FRONTMATTER);
  if (!match) return { gated: false, reason: "no docmeta" };
  let raw;
  try {
    raw = (0, import_yaml.parse)(match[1], { schema: "failsafe" });
  } catch {
  ...
  const fields = raw;
  const audience = fields.audience === "agent" || fields.audience === "human" ? fields.audience : inferAudience(relPath);
...
function detect({ changedFiles, index }) {
  ...
  for (const entry of index.entries) {
    ...
    const isMatch = (0, import_picomatch.default)(entry.meta.covers);
...
function brokenLink(doc, reason) {
  return { doc, kind: "broken-link", severity: "fail", reason };
}
function checkInternalLinks(docs, existingFiles) {
  const findings = [];
  const slugsByPath = new Map(docs.map((d) => [d.path, headingSlugs(d.content)]));
  for (const doc of docs) {
    const dir = import_node_path2.posix.dirname(doc.path);
    for (const m of doc.content.matchAll(LINK_RE)) {
      ...
      const resolved = import_node_path2.posix.normalize(import_node_path2.posix.join(dir, pathPart));
...
// src/structural-fingerprint.ts
var sortUnique = (xs) => [...new Set(xs)].sort();
...
function diffCategory(prev, curr, added, removed) {
...
function macroFindings(changes, macroDocs, changedFiles) {
  if (changes.length === 0) return [];
  const changed = new Set(changedFiles);
  const reason = `repo structure changed (${summarize(changes)}); review this orientation doc`;
  return macroDocs.filter((doc) => !changed.has(doc)).map((doc) => ({ doc, kind: "macro-stale", severity: "warn", reason }));
}
...
var isMacroDoc = (path) => MACRO_PATTERNS.some((re) => re.test(path));
function readShape(repoRoot) {
  ...
  let bin = [];
  try {
    const pkg = JSON.parse((0, import_node_fs3.readFileSync)((0, import_node_path4.join)(repoRoot, "package.json"), "utf8"));
...
function readHeadShape(repoRoot) {
  try {
    const tree = (0, import_node_child_process.execFileSync)("git", ["ls-tree", "HEAD"], { cwd: repoRoot, encoding: "utf8", stdio: ["ignore", "pipe", "ignore"] });
    const topLevel = [];
    ...
    let bin = [];
    try {
      const pkgRaw = (0, import_node_child_process.execFileSync)("git", ["show", "HEAD:package.json"], { cwd: repoRoot, encoding: "utf8", stdio: ["ignore", "pipe", "ignore"] });
...
function runGate(repoRoot, changedFiles, opts = {}) {
  ...
  const changedDocs = changedFiles.filter((f) => f.endsWith(".md") && existingFiles.has(f)).map((f) => ({ path: f, content: (0, import_node_fs3.readFileSync)((0, import_node_path4.join)(repoRoot, ...f.split("/")), "utf8") }));
  const prev = opts.previousFingerprint === void 0 ? readHeadShape(repoRoot) : opts.previousFingerprint;
  const structural = prev ? macroFindings(
