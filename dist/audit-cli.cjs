...
function externalUrlsFrom(docs) {
...
// src/url-health.ts
async function urlHealth(urls, opts) {
...
function deadlineFor(entry) {
  ...
  const deadline = new Date(reviewedDate.getTime() + days * 864e5);
...
async function runAudit(repoRoot, opts = {}) {
  const now = opts.now ?? /* @__PURE__ */ new Date();
  const fetch2 = opts.fetch ?? (async () => null);
  const index = buildIndex(repoRoot);
  const existing = listFiles(repoRoot);
  const codeFiles = opts.codeFiles ?? defaultCodeFiles(existing);
  const allDocs = [...existing].filter((p) => p.endsWith(".md")).map((p) => ({ path: p, content: (0, import_node_fs3.readFileSync)((0, import_node_path4.join)(repoRoot, ...p.split("/")), "utf8") }));
  const overdue = index.entries.flatMap((entry) => {
...
// src/write-frontmatter.ts
var HAS_FRONTMATTER = /^---\r?\n/;
function emitCovers(covers) {
  const items = covers.map((c) => `"${c.replace(/"/g, '\\"')}"`);
  return `[${items.join(", ")}]`;
}
function writeFrontmatter(content, init) {
  if (HAS_FRONTMATTER.test(content)) return content;
  const lines = ["---", `audience: ${init.audience}`, `covers: ${emitCovers(init.covers)}`];
  if (init.synced !== void 0) lines.push(`synced: ${init.synced}`);
  if (init.reviewed !== void 0) lines.push(`reviewed: ${init.reviewed}`);
  if (init.reviewInterval !== void 0) lines.push(`review_interval: ${init.reviewInterval}`);
  lines.push("---", "");
  return `${lines.join("\n")}
${content}`;
}
...
// src/init-docmeta.ts
function initDocmeta(repoRoot, opts = {}) {
  const index = buildIndex(repoRoot);
  const proposals = index.ungated.map((u) => ({
    path: u.path,
  ...
  if (!opts.dryRun) {
    for (const { path, init } of proposals) {
      const full = (0, import_node_path5.join)(repoRoot, ...path.split("/"));
      const original = (0, import_node_fs4.readFileSync)(full, "utf8");
      const updated = writeFrontmatter(original, init);
      if (updated !== original) {
        (0, import_node_fs4.writeFileSync)(full, updated);
        written.push(path);
      }
    }
  }
