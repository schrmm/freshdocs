...
2. Gather the current findings:
   - Run `doc-gate` against the staged change set (or the diff being prepared). Capture its formatted report lines.
...
3. Walk gate report lines in emitted order (drift → broken links → macro staleness → uncovered). For audit reports, handle sections in the skill order: overdue reviews and external links after drift/link/macro repairs.
