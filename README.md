...
Shared repo-shape defaults live in `repo-policy`: ignored dependency/build/cache directories, generated bytecode suffixes, docmeta-excluded Markdown surfaces (`AGENTS.md`, `docs/prd/**`, `skills/**`), and the conventional code-surface prefixes (`src/`, `lib/`, `app/`, `packages/`, `scripts/`). There is no required config file in v1; those defaults are deliberately boring and hardcoded.
