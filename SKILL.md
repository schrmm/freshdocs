...
**Bin resolution.** Both bins resolve in this order: (1) on PATH (`doc-gate`, `freshdocs-audit` — optional npm install); (2) `node_modules/.bin/`; (3) `node .agents/skills/freshdocs/dist/cli-main.cjs` / `audit-cli.cjs` for a project skill install; (4) `node ~/.codex/skills/freshdocs/dist/cli-main.cjs` / `audit-cli.cjs` for a Codex global install; (5) `node ~/.agents/skills/freshdocs/dist/cli-main.cjs` / `audit-cli.cjs` for portable global installs. Use the first one that works.
