...
`buildIndex` walks the repo recursively, skipping the shared `IGNORED_DIRS` from `repo-policy.ts`. Only `.md` files are read. Paths are returned as repo-relative POSIX (`/`-joined, regardless of host separator).
