...
`buildIndex` walks the repo recursively, skipping the shared `IGNORED_DIRS` and docmeta-excluded markdown surfaces from `repo-policy.ts`. Excluded docmeta surfaces include `AGENTS.md`, `docs/prd/**`, and `skills/**`; those files may contain agent/package metadata frontmatter that is not freshdocs docmeta. Only non-excluded `.md` files are read. Paths are returned as repo-relative POSIX (`/`-joined, regardless of host separator).
...
const IGNORED_FILE_SUFFIXES = [".pyc", ".pyo", ".pyd"] as const;
function isIgnoredFile(name: string): boolean;
const DEFAULT_CODE_PREFIXES = ["src/", "lib/", "app/", "packages/", "scripts/"] as const;
function isDocmetaIgnoredPath(path: string): boolean;
```

- `IGNORED_DIRS` is shared by the markdown walker, whole-repo file walker, and macro-shape reader. It excludes dependency, generated build output, and common Python cache/venv directories so first-run coverage does not count artifacts as source.
- `isIgnoredFile` excludes generated bytecode files such as `.pyc`, `.pyo`, and `.pyd` when walking repo files or docs.
- `DEFAULT_CODE_PREFIXES` is shared by audit coverage and per-commit uncovered-file detection. `scripts/` is included so small skill/tooling repos with their implementation in scripts are visible on first audit.
- `isDocmetaIgnoredPath` excludes markdown surfaces that should not get freshdocs frontmatter, such as PRDs, skill package files, and root agent instructions.
