...
Public Agent Skill entrypoints for `npx skills@latest add schrmm/freshdocs`.

That command lets `skills.sh` discover these skills and prompt for which ones to install into the selected project or global scope. No extra agent configuration is needed after the installer finishes; restart the target agent if it was already running so it reloads its skill directory.
